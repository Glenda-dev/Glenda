...
use crate::hal::drivers::{FB, FbDriver, INTC, IntcDriver, UART, UartDriver};
...
pub fn parse(fdt: &fdt::Fdt) {
    ...
    let timer_frequency = fdt
        ...
        .and_then(|prop| {
            if prop.value.len() == 4 {
                ...
                Some(usize::from_be_bytes(prop.value.try_into().unwrap()) as usize)
            } else {
                None
            }
        })
    ...
    crate::hal::timer::init(timer_frequency);
