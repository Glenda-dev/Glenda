...
pub fn run(hartid: usize) {
    ...
    if hartid == 0 {
        ...
        printk!("[TEST] VM test start ({} harts)\n", VM_BARRIER.total());
    }
    ...
    if VM_BARRIER.finish_and_last() {
        printk!("{}[PASS]{} VM test ({} harts)\n", ANSI_GREEN, ANSI_RESET, VM_BARRIER.total());
    }
}
...
fn vm_mapping_test() {
    ...
    // 3. 建立映射
    printk!("Mapping VA 0x{:x} -> PA 0x{:x} (R W)\n", va_1, pa_1);
    vm::mappages(table, va_1, pa_1, PGSIZE, PTE_R | PTE_W);
    printk!("Mapping VA 0x{:x} -> PA 0x{:x} (R W X)\n", va_2, pa_2);
